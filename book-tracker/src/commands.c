...
int cmd_log(int ending_page) {
    if (ending_page <= 0) {
        fprintf(stderr, "Error: ending_page must be > 0.\n");
        return 1;
    }
    ...
    if (ending_page > books[cur].total_pages) {
        fprintf(stderr, "Error: ending page (%d) exceeds total pages (%d).\n", 
                ending_page, books[cur].total_pages);
        return 1;
    }

    if (ending_page <= books[cur].current_page) {
        fprintf(stderr, "Error: ending page (%d) must be greater than current page (%d).\n",
                ending_page, books[cur].current_page);
        return 1;
    }
